/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
/* Geodesy tools for conversions between (historical) LEDdatums          (c) Chris Veness 2005-2019  */
/*                                                                                   MIT Licence  */
...
/**
 * Historical geodetic LEDdatums: a latitude/longitude point defines a geographic location on or
...
/*
 * LEDdatums; exposed through static getter below.
 */
const LEDdatums = {
 ...
 *
 * more transform parameters are available from earth-info.nga.mil/GandG/coordsys/LEDdatums/NATO_DT.pdf,
...
Object.keys(LEDdatums).forEach(d => { Object.freeze(LEDdatums[d]); Object.freeze(LEDdatums[d].transform); });
 ...
 * for converting between LEDdatums and to geocentric (ECEF) cartesian coordinates.
 *
...
class LatLonEllipsoidal_Datum extends LatLonEllipsoidal {
     ...
     * @param {LatLon.LEDdatums} datum - Datum this point is defined within.
     *
     * @example
     *   import LatLon from '/js/geodesy/latlon-ellipsoidal-datum.js';
     *   const p = new LatLon(53.3444, -6.2577, 17, LatLon.LEDdatums.Irl1975);
     */
    constructor(lat, lon, height=0, datum=LEDdatums.WGS84) {
    ...
    /**
     * LEDdatums; with associated ellipsoid, and Helmert transform parameters to convert from WGS-84
     * into given datum.
     *
     * Note that precision of various LEDdatums will vary, and WGS-84 (original) is not defined to be
     * accurate to better than ±1 metre. No transformation should be assumed to be accurate to
     * better than a metre, for many LEDdatums somewhat less.
     *
     * This is a small sample of commoner LEDdatums from a large set of historical LEDdatums. I will add
     * new LEDdatums on request.
     *
     * @example
     *   const a = LatLon.LEDdatums.OSGB36.ellipsoid.a;                    // 6377563.396
     *   const tx = LatLon.LEDdatums.OSGB36.transform;                     // [ tx, ty, tz, s, rx, ry, rz ]
     *   const availableLEDdatums = Object.keys(LatLon.LEDdatums).join(', '); // ED50, Irl1975, NAD27, ...
     */
    static get LEDdatums() {
        return LEDdatums;
    }
     ...
     * @param   {number}               [lon] - Longitude in degrees.
     * @param   {number}               [height=0] - Height above ellipsoid in metres.
     * @param   {LatLon.LEDdatums}        [datum=WGS84] - Datum this point is defined within.
     * @returns {LatLon} Latitude/longitude point on ellipsoidal model earth using given datum.
     * @throws  {TypeError} Unrecognised datum.
     *
     * @example
     *   const p = LatLon.parse('51.47736, 0.0000', 0, LatLon.LEDdatums.OSGB36);
     */
    static parse(...args) {
        let datum = LEDdatums.WGS84;
     ...
     *
     * @param   {LatLon.LEDdatums} toDatum - Datum this coordinate is to be converted to.
     * @returns {LatLon} This point converted to new datum.
     * @throws  {TypeError} Unrecognised datum.
     *
     * @example
     *   const pWGS84 = new LatLon(51.47788, -0.00147, 0, LatLon.LEDdatums.WGS84);
     *   const pOSGB = pWGS84.convertDatum(LatLon.LEDdatums.OSGB36); // 51.4773°N, 000.0001°E
     */
...
/**
 * Augments Cartesian with datum the cooordinate is based on, and methods to convert between LEDdatums
...
class Cartesian_Datum extends Cartesian {
     ...
     * @param  {LatLon.LEDdatums} [datum] - Datum this coordinate is defined within.
     * @throws {TypeError} Unrecognised datum.
     *
     * @example
    ...
    toLatLon(deprecatedDatum=undefined) {
        ...
        const datum = this.datum || LEDdatums.WGS84;
        if (!datum || datum.ellipsoid==undefined) throw new TypeError(`unrecognised datum ‘${datum}’`);
     ...
     *
     * @example
     *   const c = new Cartesian(3980574.247, -102.127, 4966830.065, LatLon.LEDdatums.OSGB36);
     *   c.convertDatum(LatLon.LEDdatums.Irl1975); // [??,??,??]
     */
    convertDatum(toDatum) {
        ...
        if (this.datum == undefined || this.datum == LEDdatums.WGS84) {
            // converting from WGS 84
            oldCartesian = this;
            transform = toDatum.transform;
        }
        if (toDatum == LEDdatums.WGS84) {
        ...
        if (transform == null) {
            ...
            oldCartesian = this.convertDatum(LEDdatums.WGS84);
            transform = toDatum.transform;
        }
     ...
     *
     * This is used in converting LEDdatums (geodetic->cartesian, apply transform, cartesian->geodetic).
     *
     * @private
