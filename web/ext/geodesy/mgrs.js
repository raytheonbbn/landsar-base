...
class Mgrs {
    ...
    toUtm() {
        ...
        const nBand = Math.floor(new LatLon_Utm(latBand, 3).utm_toUtm().northing/100e3)*100e3;
...
class Utm_Mgrs extends Utm {
    ...
    toMgrs() {
        ...
        return new Mgrs(zone, band, e100k, n100k, Math.round(easting), Math.round(northing));
    }
...
/**
 * Extends LatLon_Utm adding toMgrs() method to the Utm object returned by LatLon.toUtm().
 *
 * @extends LatLon_Utm
 */
class Latlon_Utm_Mgrs extends LatLon_Utm {
    ...
    toUtm(zoneOverride=undefined) {
        const utm = super.utm_toUtm(zoneOverride);
